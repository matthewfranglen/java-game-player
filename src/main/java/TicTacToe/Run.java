...
public class Run {
    ...
    private static void getMove(Board board, Game game) throws IOException {
        ...
        try {
            ...
            if (!currentBoard[i][j].getCell().equals("[ ]")) {
                ...
                currentBoard[i][j].setCell(game.getOpponentPiece());
    ...
    private static void makeMove(Board board, Game game) {
        ...
        int[] playerMove = aiPlayer.miniMax(board, game.getDifficultyLevel(), game.getAiPlayerPiece(), game);
        int i = playerMove[1];
        int j = playerMove[2];
        ...
        if (!currentBoard[i][j].getCell().equals("[ ]")) {
            ...
            currentBoard[i][j].setCell(game.getAiPlayerPiece());
            board.setBoard(currentBoard);
            board.printBoard();
        }
    }
